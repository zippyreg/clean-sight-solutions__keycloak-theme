...
import { FormPanel, TextControl } from "../../shared/keycloak-ui-shared";
...
export const RealmSettingsSCIMTab = ({ realm, save }: RealmSettingsSCIMTabProps) => {
    ...
    const [isEnabled, _, setEnable] = useToggle(
        realm.attributes?.["scim.enabled"] === "true"
    );

    const watchAuthMode = watch(
        convertAttributeNameToForm("attributes.scim.authentication.mode"),
        "KEYCLOAK"
    );

    const toggleKey = (key: string) => {
        return key.startsWith("_") ? key.substring(1) : `_${key}`;
    };
    ...
    const toggleScim = () => {
        ...
        for (const key in attributes) {
            if (
                key.startsWith(newIsEnabled ? "_scim." : "scim") &&
            ...
            <FormProvider {...form}>
                ...
                <FormPanel title={t("scim.modes")} className="kc-scim-template">
                    ...
                    >
                        ...
                        <TextControl
                            name={convertAttributeNameToForm(
                                "attributes.scim.external.jwks.uri"
                            )}
                        ...
                        <TextControl
                            name={convertAttributeNameToForm(
                                "attributes.scim.external.audience"
                            )}
